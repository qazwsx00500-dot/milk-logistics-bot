...
def make_sample(path):
    ...
    ws = wb.active
    ws.title = "每日配送"
    ws.append(["車號", "店家名稱", "店家地址", "瓶數", "油資單價", "特殊需求"])
    sample = [
        ...
        # 特殊需求示範（可留空）：10前=上午10點前要到；14後=下午2點後才送；首站=該車第一站；末站=最後一站
        ("車01", "大雅早市店", "台中市大雅區民生路1號", 22, None, "10前,首站"),
        ("車02", "后里晚市", "台中市后里區甲后路二段1號", 19, None, "14後,末站"),
    ]
    for r in sample:
        ws.append(list(r))
    ...
    notes = [
        ...
        ["特殊需求", "選填", "店家特殊配送要求，可留空。支援：①時間窗 如「10前」(上午10點前到)、「14後」(下午2點後才送)、「10-14」(只能這時段)；②固定順位 「首站」(該車第一站就送，如冷鏈怕熱)、「末站」(最後一站才送，如營業晚)。多條用逗號分隔，例：『10前,首站』。報表/派車單會標⏰徽章，排不進時間窗會顯示⚠警示"],
        ["", "", ""],
